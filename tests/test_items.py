...
def test_article_item_titles_empty():
    with pytest.raises(ValueError):
        ArticleItem(title="", url="http://onet.pl/valid", date="2026-01-01")


def test_article_item_url_invalid():
    with pytest.raises(ValueError):
        ArticleItem(title="Title", url="ftp://invalid-url.com", date="2026-01-01")
...
def test_article_item_url_https():
    ...
    item = ArticleItem(title="Title", url="https://onet.pl/article", date="2026-01-01")
...
def test_article_item_title_strips_whitespace():
    ...
    item = ArticleItem(title="  Title with spaces  ", url="http://onet.pl/valid", date="2026-01-01")
    assert item.title == "Title with spaces"


def test_article_item_read_time_positive():
    """Read time should accept positive integers."""
    item = ArticleItem(title="Title", url="http://onet.pl/valid", date="2026-01-01", read_time=5)
