...
MOCK_RULES = {"scam_phrases": ["CLICK HERE", "BUY NOW"], "cutoff_markers": ["READ MORE"]}


@pytest.fixture
def mock_rules():
    with patch("onet_scraper.utils.text_cleaners.load_cleaning_rules", return_value=MOCK_RULES):
        yield
...
def test_clean_article_content_removes_scam(mock_rules):
    raw = ["Normal content", "This is a scam CLICK HERE to win", "More normal content"]
    result = clean_article_content(raw)
...
def test_clean_article_content_cutoff(mock_rules):
    raw = ["Start of article", "Middle part", "READ MORE below", "This should be cut off"]
    result = clean_article_content(raw)
